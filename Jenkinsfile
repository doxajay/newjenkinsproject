    ...
    stages {
        ...
        stage('Push Docker Image to ECR') {
            steps {
                withAWS(credentials: 'aws-cred', region: 'us-east-1') {
                    ...
                    sh 'docker tag projectjenkins:latest 460982569648.dkr.ecr.us-east-1.amazonaws.com/ecrnew:latest'
        ...
        stage('Integrate Jenkins with EKS Cluster and Deploy App') {
            steps {
                withAWS(credentials: 'aws-cred', region: 'us-east-1') {
                  script {
                    sh ('aws eks --region us-east-1 update-kubeconfig --name eksnew')
